...
TMP_DIR="${ROOT_DIR}/.tmp/lower-structural-native-append-child-module"
  ...
  Let(name=makeArgs) {
    Fn(params=value) {
      Block {
        Return {
          AppendChild {
            MakeBlock { Lit(value="spawn-args") }
            MakeLitString { Lit(value="value") Var(name=value) }
          }
        }
      }
    }
  ...
  Let(name=start) {
    Fn() {
      Block {
        ...
        Let(name=plan) { Call(target=lower.emitStructuralRecordPlanWithSymbols) { ChildAt { Var(name=records) Lit(value=0) } Var(name=records) } }
        If {
...
echo 'lower structural native append child module: PASS'
