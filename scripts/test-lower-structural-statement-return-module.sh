...
TMP_DIR="${ROOT_DIR}/.tmp/lower-structural-statement-return-module"
  ...
  Let(name=classify) {
    Fn(params=value) {
      Block {
        ...
        If {
          Eq { Var(name=value) Lit(value=2) }
          Block { Return { Lit(value=20) } }
          Block { Lit(value=0) }
        }
        Return { Lit(value=30) }
      }
...
echo 'lower structural statement return module: PASS'
