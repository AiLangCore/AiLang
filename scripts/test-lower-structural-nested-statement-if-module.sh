...
TMP_DIR="${ROOT_DIR}/.tmp/lower-structural-nested-statement-if-module"
      ...
      Block {
        If {
          ...
          Block {
            ...
            Call(target=sys.stdout.writeLine) { Lit(value=2) }
          }
          Block { Lit(value=0) }
        }
        Return { Lit(value=0) }
...
echo 'lower structural nested statement if module: PASS'
