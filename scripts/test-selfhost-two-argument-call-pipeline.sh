...
TMP_DIR="${ROOT_DIR}/.tmp/selfhost-two-argument-call-pipeline"
...
Program {
  Project(name="two-argument-call-pipeline" entryFile="src/app.aos" entryExport="start")
}
AOS
...
Program {
  ...
  Let(name=start) {
    Fn() {
      Block { Return { Call(target=first) { Lit(value=5) Lit(value=7) } } }
    }
  }
}
AOS
...
echo "self-hosted two argument call pipeline: PASS"
