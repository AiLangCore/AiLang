...
static int emit_wasm_spa_files(const char* out_dir)
{
    ...
    }
    {
        const char* main_js_head =
            ...
            "  const state = { host, svg, width, height, frameParts: [], eventQueue: [], lastPolledEvent: { type: 'none', x: 0, y: 0 } };\n"
            "  const pushEvent = (type, x, y) => { state.eventQueue.push({ type, x: x|0, y: y|0 }); if (state.eventQueue.length > 64) state.eventQueue.shift(); };\n"
            "  svg.addEventListener('click', (ev) => { pushEvent('click', ev?.offsetX ?? 0, ev?.offsetY ?? 0); });\n"
            "  svg.addEventListener('mousemove', (ev) => { pushEvent('move', ev?.offsetX ?? 0, ev?.offsetY ?? 0); });\n"
            "  svg.addEventListener('keydown', () => { pushEvent('keydown', 0, 0); });\n"
        ...
        const char* main_js_tail =
            ...
            "  const win = uiState.windows.get(windowId);\n"
            "  if (!win) return -1;\n"
            "  const evt = win.eventQueue.length > 0 ? win.eventQueue.shift() : { type: 'none', x: 0, y: 0 };\n"
            "  win.lastPolledEvent = evt;\n"
            "  if (evt.type === 'click') return 1;\n"
            "  if (evt.type === 'move') return 2;\n"
            "  if (evt.type === 'keydown') return 3;\n"
            "  return 0;\n"
            "};\n"
            "globalThis.__aivmUiPollEventX = (windowId) => {\n"
            "  const win = uiState.windows.get(windowId);\n"
            "  if (!win) return -1;\n"
            "  return (win.lastPolledEvent?.x | 0) || 0;\n"
            "};\n"
            "globalThis.__aivmUiPollEventY = (windowId) => {\n"
            "  const win = uiState.windows.get(windowId);\n"
            "  if (!win) return -1;\n"
            "  return (win.lastPolledEvent?.y | 0) || 0;\n"
            "};\n"
