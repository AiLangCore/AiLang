...
public static class VmSyscallDispatcher
{
    public static bool TryGetExpectedArity(SyscallId id, out int arity)
    {
        arity = id switch
        {
            ...
            SyscallId.UiDrawTextPath => 5,
            SyscallId.UiDrawRectPaint => 9,
            SyscallId.UiDrawEllipsePaint => 9,
            SyscallId.UiDrawPolylinePaint => 5,
            SyscallId.UiDrawPolygonPaint => 6,
            SyscallId.UiDrawPathPaint => 7,
            SyscallId.UiDrawTextPaint => 7,
            SyscallId.UiFilterBlur => 7,
            SyscallId.UiGroupPush => 1,
            SyscallId.UiGroupPop => 1,
            SyscallId.UiTranslate => 3,
            SyscallId.UiScale => 3,
            SyscallId.UiRotate => 2,
    ...
    public static bool TryInvoke(SyscallId id, ReadOnlySpan<SysValue> args, VmNetworkState network, out SysValue result)
    {
        ...
        switch (id)
        {
            ...
            case SyscallId.UiDrawTextPath:
                if (!TryGetInt(args, 0, 5, out var uiTextPathHandle) ||
                    !TryGetString(args, 1, 5, out var uiTextPathPath) ||
                    !TryGetString(args, 2, 5, out var uiTextPathText) ||
                    !TryGetString(args, 3, 5, out var uiTextPathColor) ||
                    !TryGetInt(args, 4, 5, out var uiTextPathSize))
                {
                    return true;
                }
                VmSyscalls.UiDrawTextPath(uiTextPathHandle, uiTextPathPath, uiTextPathText, uiTextPathColor, uiTextPathSize);
                result = SysValue.Void();
                return true;

            case SyscallId.UiDrawRectPaint:
                if (!TryGetInt(args, 0, 9, out var uiRectPaintHandle) ||
                    !TryGetInt(args, 1, 9, out var uiRectPaintX) ||
                    !TryGetInt(args, 2, 9, out var uiRectPaintY) ||
                    !TryGetInt(args, 3, 9, out var uiRectPaintW) ||
                    !TryGetInt(args, 4, 9, out var uiRectPaintH) ||
                    !TryGetString(args, 5, 9, out var uiRectPaintFill) ||
                    !TryGetString(args, 6, 9, out var uiRectPaintStroke) ||
                    !TryGetInt(args, 7, 9, out var uiRectPaintStrokeWidth) ||
                    !TryGetInt(args, 8, 9, out var uiRectPaintOpacity))
                {
                    return true;
                }
                VmSyscalls.UiDrawRectPaint(uiRectPaintHandle, uiRectPaintX, uiRectPaintY, uiRectPaintW, uiRectPaintH, uiRectPaintFill, uiRectPaintStroke, uiRectPaintStrokeWidth, uiRectPaintOpacity);
                result = SysValue.Void();
                return true;

            case SyscallId.UiDrawEllipsePaint:
                if (!TryGetInt(args, 0, 9, out var uiEllipsePaintHandle) ||
                    !TryGetInt(args, 1, 9, out var uiEllipsePaintX) ||
                    !TryGetInt(args, 2, 9, out var uiEllipsePaintY) ||
                    !TryGetInt(args, 3, 9, out var uiEllipsePaintW) ||
                    !TryGetInt(args, 4, 9, out var uiEllipsePaintH) ||
                    !TryGetString(args, 5, 9, out var uiEllipsePaintFill) ||
                    !TryGetString(args, 6, 9, out var uiEllipsePaintStroke) ||
                    !TryGetInt(args, 7, 9, out var uiEllipsePaintStrokeWidth) ||
                    !TryGetInt(args, 8, 9, out var uiEllipsePaintOpacity))
                {
                    return true;
                }
                VmSyscalls.UiDrawEllipsePaint(uiEllipsePaintHandle, uiEllipsePaintX, uiEllipsePaintY, uiEllipsePaintW, uiEllipsePaintH, uiEllipsePaintFill, uiEllipsePaintStroke, uiEllipsePaintStrokeWidth, uiEllipsePaintOpacity);
                result = SysValue.Void();
                return true;

            case SyscallId.UiDrawPolylinePaint:
                if (!TryGetInt(args, 0, 5, out var uiPolylinePaintHandle) ||
                    !TryGetString(args, 1, 5, out var uiPolylinePaintPoints) ||
                    !TryGetString(args, 2, 5, out var uiPolylinePaintStroke) ||
                    !TryGetInt(args, 3, 5, out var uiPolylinePaintStrokeWidth) ||
                    !TryGetInt(args, 4, 5, out var uiPolylinePaintOpacity))
                {
                    return true;
                }
                VmSyscalls.UiDrawPolylinePaint(uiPolylinePaintHandle, uiPolylinePaintPoints, uiPolylinePaintStroke, uiPolylinePaintStrokeWidth, uiPolylinePaintOpacity);
                result = SysValue.Void();
                return true;

            case SyscallId.UiDrawPolygonPaint:
                if (!TryGetInt(args, 0, 6, out var uiPolygonPaintHandle) ||
                    !TryGetString(args, 1, 6, out var uiPolygonPaintPoints) ||
                    !TryGetString(args, 2, 6, out var uiPolygonPaintFill) ||
                    !TryGetString(args, 3, 6, out var uiPolygonPaintStroke) ||
                    !TryGetInt(args, 4, 6, out var uiPolygonPaintStrokeWidth) ||
                    !TryGetInt(args, 5, 6, out var uiPolygonPaintOpacity))
                {
                    return true;
                }
                VmSyscalls.UiDrawPolygonPaint(uiPolygonPaintHandle, uiPolygonPaintPoints, uiPolygonPaintFill, uiPolygonPaintStroke, uiPolygonPaintStrokeWidth, uiPolygonPaintOpacity);
                result = SysValue.Void();
                return true;

            case SyscallId.UiDrawPathPaint:
                if (!TryGetInt(args, 0, 7, out var uiPathPaintHandle) ||
                    !TryGetString(args, 1, 7, out var uiPathPaintPath) ||
                    !TryGetString(args, 2, 7, out var uiPathPaintFill) ||
                    !TryGetString(args, 3, 7, out var uiPathPaintStroke) ||
                    !TryGetInt(args, 4, 7, out var uiPathPaintStrokeWidth) ||
                    !TryGetInt(args, 5, 7, out var uiPathPaintOpacity) ||
                    !TryGetInt(args, 6, 7, out var uiPathPaintClosed))
                {
                    return true;
                }
                VmSyscalls.UiDrawPathPaint(uiPathPaintHandle, uiPathPaintPath, uiPathPaintFill, uiPathPaintStroke, uiPathPaintStrokeWidth, uiPathPaintOpacity, uiPathPaintClosed);
                result = SysValue.Void();
                return true;

            case SyscallId.UiDrawTextPaint:
                if (!TryGetInt(args, 0, 7, out var uiTextPaintHandle) ||
                    !TryGetInt(args, 1, 7, out var uiTextPaintX) ||
                    !TryGetInt(args, 2, 7, out var uiTextPaintY) ||
                    !TryGetString(args, 3, 7, out var uiTextPaintText) ||
                    !TryGetString(args, 4, 7, out var uiTextPaintColor) ||
                    !TryGetInt(args, 5, 7, out var uiTextPaintSize) ||
                    !TryGetInt(args, 6, 7, out var uiTextPaintOpacity))
                {
                    return true;
                }
                VmSyscalls.UiDrawTextPaint(uiTextPaintHandle, uiTextPaintX, uiTextPaintY, uiTextPaintText, uiTextPaintColor, uiTextPaintSize, uiTextPaintOpacity);
                result = SysValue.Void();
                return true;

            case SyscallId.UiFilterBlur:
                if (!TryGetInt(args, 0, 7, out var uiBlurHandle) ||
                    !TryGetString(args, 1, 7, out var uiBlurPath) ||
                    !TryGetString(args, 2, 7, out var uiBlurColor) ||
                    !TryGetInt(args, 3, 7, out var uiBlurStrokeWidth) ||
                    !TryGetInt(args, 4, 7, out var uiBlurRadius) ||
                    !TryGetInt(args, 5, 7, out var uiBlurOpacity) ||
                    !TryGetInt(args, 6, 7, out var uiBlurClosed))
                {
                    return true;
                }
                VmSyscalls.UiFilterBlur(uiBlurHandle, uiBlurPath, uiBlurColor, uiBlurStrokeWidth, uiBlurRadius, uiBlurOpacity, uiBlurClosed);
                result = SysValue.Void();
                return true;

            case SyscallId.UiGroupPush:
                if (!TryGetInt(args, 0, 1, out var uiGroupPushHandle))
                {
                    return true;
                }
                VmSyscalls.UiGroupPush(uiGroupPushHandle);
                result = SysValue.Void();
                return true;

            case SyscallId.UiGroupPop:
                if (!TryGetInt(args, 0, 1, out var uiGroupPopHandle))
                {
                    return true;
                }
                VmSyscalls.UiGroupPop(uiGroupPopHandle);
                result = SysValue.Void();
                return true;

            case SyscallId.UiTranslate:
                if (!TryGetInt(args, 0, 3, out var uiTranslateHandle) ||
                    !TryGetInt(args, 1, 3, out var uiTranslateDx) ||
                    !TryGetInt(args, 2, 3, out var uiTranslateDy))
                {
                    return true;
                }
                VmSyscalls.UiTranslate(uiTranslateHandle, uiTranslateDx, uiTranslateDy);
                result = SysValue.Void();
                return true;

            case SyscallId.UiScale:
                if (!TryGetInt(args, 0, 3, out var uiScaleHandle) ||
                    !TryGetInt(args, 1, 3, out var uiScaleX) ||
                    !TryGetInt(args, 2, 3, out var uiScaleY))
                {
                    return true;
                }
                VmSyscalls.UiScale(uiScaleHandle, uiScaleX, uiScaleY);
                result = SysValue.Void();
                return true;

            case SyscallId.UiRotate:
                if (!TryGetInt(args, 0, 2, out var uiRotateHandle) ||
                    !TryGetInt(args, 1, 2, out var uiRotateDegrees))
                {
                    return true;
                }
                VmSyscalls.UiRotate(uiRotateHandle, uiRotateDegrees);
                result = SysValue.Void();
                return true;
