...
TMP_DIR="${ROOT_DIR}/.tmp/lower-structural-local-if-call-module"
rm -rf "${TMP_DIR}"
mkdir -p "${TMP_DIR}"

cat > "${TMP_DIR}/build.aos" <<AOS
Program { Import(path="../../src/compiler/parser.aos") Import(path="../../src/compiler/lower.aos") Import(path="../../src/compiler/object_linker.aos") Export(name=start) Let(name=start) { Fn() { Block { Let(name=source) { Lit(value="Program { Let(name=start) { Fn() { Block { Return { Call(target=choose) { } } } } } Let(name=choose) { Fn() { Block { Let(name=value) { Lit(value=41) } If { Lit(value=true) Block { Return { Call(target=echo) { Var(name=value) } } } Block { Return { Call(target=echo) { Lit(value=9) } } } } } } } Let(name=echo) { Fn(params=value) { Block { Return { Var(name=value) } } } } }") } Let(name=program) { Call(target=parse.parseDocument) { Var(name=source) } } Let(name=records) { Call(target=lower.collectFunctionRecords) { Var(name=program) Lit(value="app.aos") } } Let(name=plans) { Call(target=lower.emitStructuralPlansWithSymbols) { Var(name=records) Lit(value=0) MakeBlock { Lit(value="plans") } } } Call(target=sys.fs.file.write) { Lit(value="${TMP_DIR}/app.aibc1") Call(target=objectLinker.emitLinkedPlansAibc1Bytes) { Var(name=plans) } } Return { Lit(value=0) } } } } }
AOS
...
echo 'lower structural local if call module: PASS'
