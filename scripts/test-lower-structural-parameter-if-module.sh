...
TMP_DIR="${ROOT_DIR}/.tmp/lower-structural-parameter-if-module"
rm -rf "${TMP_DIR}"
mkdir -p "${TMP_DIR}"

cat > "${TMP_DIR}/build.aos" <<AOS
Program { Import(path="../../src/compiler/parser.aos") Import(path="../../src/compiler/lower.aos") Import(path="../../src/compiler/object_linker.aos") Export(name=start) Let(name=start) { Fn() { Block { Let(name=source) { Lit(value="Program { Let(name=main) { Fn() { Block { Return { Call(target=choose) { Lit(value=true) } } } } } Let(name=choose) { Fn(params=enabled) { Block { If { Var(name=enabled) Block { Return { Lit(value=42) } } Block { Return { Lit(value=0) } } } } } } }") } Let(name=program) { Call(target=parse.parseDocument) { Var(name=source) } } Let(name=records) { Call(target=lower.collectFunctionRecords) { Var(name=program) Lit(value="app.aos") } } Let(name=mainPlan) { Call(target=lower.emitStructuralRecordPlan) { ChildAt { Var(name=records) Lit(value=0) } } } Let(name=choosePlan) { Call(target=lower.emitStructuralRecordPlan) { ChildAt { Var(name=records) Lit(value=1) } } } Let(name=plans) { AppendChild { AppendChild { MakeBlock { Lit(value="plans") } Var(name=mainPlan) } Var(name=choosePlan) } } Call(target=sys.fs.file.write) { Lit(value="${TMP_DIR}/app.aibc1") Call(target=objectLinker.emitLinkedPlansAibc1Bytes) { Var(name=plans) } } Return { Lit(value=0) } } } } }
AOS
...
echo 'lower structural parameter if module: PASS'
