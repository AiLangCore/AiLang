...
TMP_DIR="${ROOT_DIR}/.tmp/object-linker-block-expression"
  ...
  Let(name=start) {
    Fn() {
      Block {
        ...
        Let(name=entry1) { Call(target=blockBuilder.emit) { Var(name=entry0) Lit(value="CONST") Lit(value="int:20") } }
        Let(name=entry2) { Call(target=blockBuilder.emit) { Var(name=entry1) Lit(value="STORE_LOCAL") Lit(value="0") } }
        Let(name=entry3) { Call(target=blockBuilder.emit) { Var(name=entry2) Lit(value="LOAD_LOCAL") Lit(value="0") } }
        Let(name=entry4) { Call(target=blockBuilder.emit) { Var(name=entry3) Lit(value="CONST") Lit(value="int:22") } }
        Let(name=entry5) { Call(target=blockBuilder.emit) { Var(name=entry4) Lit(value="ADD_INT") Lit(value="") } }
        Let(name=entry6) { Call(target=blockBuilder.emit) { Var(name=entry5) Lit(value="CONST") Lit(value="int:42") } }
        Let(name=entry7) { Call(target=blockBuilder.emit) { Var(name=entry6) Lit(value="EQ_INT") Lit(value="") } }
        Let(name=entry8) { Call(target=blockBuilder.terminateBranch) { Var(name=entry7) Lit(value="then") Lit(value="else") } }
        Let(name=entry) { Call(target=blockBuilder.seal) { Var(name=entry8) } }
...
DISASM="$(cd "${ROOT_DIR}" && ./tools/ailang debug disasm "${TMP_DIR}/app.aibc1" 0 16)"
printf '%s\n' "${DISASM}" | rg -Fq $'4\tADD_INT\t0'
printf '%s\n' "${DISASM}" | rg -Fq $'6\tEQ_INT\t0'

echo 'object linker block expression: PASS'
