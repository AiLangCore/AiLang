...
TMP_DIR="${ROOT_DIR}/.tmp/selfhost-arithmetic-pipeline"
...
Program {
  Project(name="arithmetic-pipeline" entryFile="src/app.aos" entryExport="start")
}
AOS
...
Program {
  ...
  Let(name=start) {
    Fn() {
      Block {
        Let(name=left) { Lit(value=5) }
        Let(name=right) { Lit(value=7) }
        Return { Add { Var(name=left) Var(name=right) } }
      }
    }
  }
}
...
echo "self-hosted arithmetic pipeline: PASS"
