...
TMP_DIR="${ROOT_DIR}/.tmp/lower-structural-module"
rm -rf "${TMP_DIR}"
mkdir -p "${TMP_DIR}"
...
Program {
  ...
  Export(name=start)
  Let(name=start) { Fn() { Block { Let(name=source) { Lit(value="Program { Let(name=main) { Fn() { Block { Let(name=value) { Lit(value=20) } Return { Add { Var(name=value) Lit(value=22) } } } } } }") } Let(name=program) { Call(target=parse.parseDocument) { Var(name=source) } } Let(name=records) { Call(target=lower.collectFunctionRecords) { Var(name=program) Lit(value="app.aos") } } Let(name=plan) { Call(target=lower.emitStructuralRecordPlan) { ChildAt { Var(name=records) Lit(value=0) } } } Let(name=plans) { AppendChild { MakeBlock { Lit(value="plans") } Var(name=plan) } } Call(target=sys.fs.file.write) { Lit(value="${TMP_DIR}/app.aibc1") Call(target=objectLinker.emitLinkedPlansAibc1Bytes) { Var(name=plans) } } Return { Lit(value=0) } } } }
}
AOS
...
echo 'lower structural module: PASS'
