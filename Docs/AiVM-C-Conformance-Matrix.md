...
| `NODE_*`, `ATTR_*`, `CHILD_*`, `MAKE_*` | in_progress | Implemented deterministic `NODE_*`, `ATTR_*`, `CHILD_*`, `MAKE_BLOCK`, `APPEND_CHILD`, `MAKE_ERR`, `MAKE_LIT_*`; `MAKE_NODE` remains explicit unsupported. |
