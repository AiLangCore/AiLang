...
sed \
  -e 's/Lit(value=42)/Lit(value=41)/' \
  -e 's#app-equality-false.aibc1#app-equality-true.aibc1#g' \
  "${TMP_DIR}/build-equality-false.aos" > "${TMP_DIR}/build-equality-true.aos"
run_build "${TMP_DIR}/build-equality-true.aos" "${TMP_DIR}/app-equality-true.aibc1" 42

echo 'lower structural bound if call module: PASS'
