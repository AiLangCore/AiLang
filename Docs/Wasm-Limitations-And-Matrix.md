...
| vector UI syscalls (`sys.ui.*`) | native host implementation-dependent | blocked unless host implements | partial (`createWindow`/`beginFrame`/`drawRect`/`drawText`/`endFrame`/`present`/`closeWindow` via SVG; others still `RUN101`) | partial (`createWindow`/`beginFrame`/`drawRect`/`drawText`/`endFrame`/`present`/`closeWindow` via SVG; others still `RUN101`) |
